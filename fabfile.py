...
env.repository = 'git@github.com:precog/quirrelide.git' if not 'repository' in env else env.repository
env.sitepath = '%(repopath)s' % env if not 'sitepath' in env else env.sitepath
...
def deploy():
    """
        Deploy labcoat from github to server
    """
